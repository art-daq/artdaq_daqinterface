...
def bookkeeping_for_fhicl_documents_artdaq_v3_base(self):
    ...
    for i_proc in range(len(self.procinfos)):
        ...
        if "BoardReader" in self.procinfos[i_proc].name:
            ...
            unflattened_parents_of_subsystems_with_routing_masters = [self.subsystems[subsystem_id].sources for subsystem_id in self.subsystems if subsystem_id in routing_master_hostnames.keys()]
